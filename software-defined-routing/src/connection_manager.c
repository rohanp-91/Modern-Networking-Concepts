    ...
    while(TRUE){
        ...
        for(sock_index=0; sock_index<=head_fd; sock_index+=1){
            ...
            if(FD_ISSET(sock_index, &watch_list)){
                ...
                /* timer interrupt */
                if(isTimer(sock_index))
                {
                    if(isSelfTimer(sock_index))
                    {
                        send_routing_updates();
                        reassign_self_timer(sock_index);
                    }
                    else
                    {
			            printf("router crashed...\n"); 
                        update_crashed_router(sock_index);
                ...
                /* control_socket */
                else if(sock_index == control_socket){
                ...
                else
                {
                    ...
                    {
                        process_data_packet(sock_index);
                    }
                    else 
                        process_data_packet(sock_index);
                }
